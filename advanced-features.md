...
- **是什么？**AUTO_MAA私域邮箱是基于阿里云企业邮箱的`automaa.xyz`域名邮箱。
- **优点是？**嗯，域名比较特别，看起来专业点？
- **怎么用？**AUTO_MAA私域邮箱的登录页为[阿里邮箱企业版](https://qiye.aliyun.com)，SMTP 服务器地址为`smtp.qiye.aliyun.com`，授权码为`登录密码`，其他使用方法与普通邮箱一致。
- **咋申请？**发送申请邮件到`DLmaster_361@automaa.xyz`，要求邮件中包含：`申请邮箱的类别（用户/项目组）`、`邮箱名（纯英文）`、`安全手机号`、`申请原因`。邮箱创建成功后，将会返回一封包含`初始密码`的回执邮件。
...
2. 添加群机器人
    - **电脑端**：进入内部群聊，点击右上角的“三个点”菜单，选择“添加群机器人”。
    - **手机端**：在内部群聊中，点击右上角的“三个点”菜单，然后选择“添加群机器人”。
    - 机器人的名称和头像可随意填写

3. 获取群机器人 Webhook 地址
    - 机器人创建者可在查看机器人信息时获取对应的 Webhook URL。
    - **手机端**：进入群聊，点击右上角的“三个点”菜单，选择“群机器人”，点击对应机器人后即可看到 Webhook 地址。
