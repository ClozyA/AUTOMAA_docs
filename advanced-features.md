...
- **新建/删除用户**：添加或删除用户。
- **调整顺序**：改变用户的代理执行顺序，执行顺序为：优先代理 **简洁** 配置模式下的用户，优先代理序号靠前的用户。
- **显示密码**：输入管理密钥后可查看或修改密码。
- **用户仪表盘**：只用于查看用户信息，不可修改，可以复制。
...
- **简洁模式**
  ...
  3. **定时执行**、**自动更新** 保持关闭，**任务完成后行为**、**启动MAA后行为**、**MAA 最小化相关设置** 按实际配置与执行情况自动调整。
  4. 其他配置沿用 **MAA 全局设置**。
  
- **详细模式**

  1. 用户配置页展示的配置项优先生效。
  2. **定时执行** 保持关闭，**任务完成后行为**、**启动MAA后行为**、**MAA 最小化相关设置** 按实际配置与执行情况自动调整。
  3. 其他配置沿用 **用户剿灭任务具体配置**、**用户日常任务具体配置**。

### 调度策略说明

以下为 **自动代理** 模式下，AUTO_MAA 的任务调度策略。

- 每个 **用户** 包含 **剿灭任务**、**日常任务** 两个子任务。简洁模式下 **日常任务** 默认开启。软件不检查用户是否存在同时运行情况。调度顺序：**剿灭 > 日常**。
